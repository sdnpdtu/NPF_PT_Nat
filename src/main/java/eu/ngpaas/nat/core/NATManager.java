            ...
            if (isActive) {
                    ...
                    if ((arpPacket.getOpCode() == ARP.OP_REQUEST)) {
                        for (IpPrefix ipp:real2natted_ip.values()) {
                            if (Ip4Address.valueOf(arpPacket.getTargetProtocolAddress()).equals(ipp.address())) {
                                ...
                                log.info("Returned MAC: " + real_mac_to_natted_mac.get(natted_ip_to_mac.get(ipp)).toString());
                                processArpRequest(context, ethPkt, real_mac_to_natted_mac.get(natted_ip_to_mac.get(ipp)));
                                context.block();
                                break;
                            }
                        }
                ...
                // IP packet
                    ...
                    if (natted_mac_to_real_mac.keySet().contains(ethPkt.getSourceMAC())) {
                        src_host = hostService.getHost(HostId.hostId(natted_mac_to_real_mac.get(ethPkt.getSourceMAC())));
                    }
                    else {
                    ...
                    if (natted_mac_to_real_mac.keySet().contains(ethPkt.getDestinationMAC())) {
                        dst_host = hostService.getHost(HostId.hostId(natted_mac_to_real_mac.get(ethPkt.getDestinationMAC())));
                    }
                    else {
                    ...
                    if (real2natted_ip.keySet().contains(dstIp) || real2natted_ip.values().contains(dstIp)) {
                        ...
                        ForwardingObjective.Builder fwdObj = DefaultForwardingObjective.builder()
                                ...
                                .withSelector(dropSelector.build())
                                .withTreatment(dropTreatment.build())
                    ...
                    if (real2natted_ip.keySet().contains(srcIp) && pkt.receivedFrom().deviceId().equals(src_host.location().deviceId())) {
                        ...
                        log.info("Natted IP from " + srcIp.address().toString() + " to " + real2natted_ip.get(srcIp).address().toString());
    ...
    public SimpleResponse formalValidation(PolicyRule pr) {
        ...
        for (CopyOnWriteArrayList<PolicyCondition> clause: pr.getPolicyConditions()) {
            if (clause.size() > 1) {
                ...
                if (!(pc.getPolicyVariable().equalsIgnoreCase("host_ip"))) {
                    restResponse = new SimpleResponse("Formal error: Parameter " + pc.getPolicyVariable() + " invalid. Valid parameter: host_ip.", false);
                    return restResponse;
                }
            }
        }
        if (pr.getPolicyActions().size() > 1) {
            ...
            if (!(pa.getPolicyVariable().equalsIgnoreCase("natted_ip"))) {
                restResponse = new SimpleResponse("Formal error: Parameter " + pa.getPolicyVariable() + " invalid. Valid parameter: natted_ip.", false);
                return restResponse;
            }
        }
